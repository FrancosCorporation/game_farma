// Screenshot com char-preview SEM RoomEnvironment (teste de reflexo real no jogo).
import { chromium } from 'playwright';
import { createServer } from 'node:http';
import { readFileSync, existsSync, statSync } from 'node:fs';
import { join, extname } from 'node:path';
const model = process.argv[2] || 'ana';
const out = process.argv[3] || '/tmp/opencode/shot_noenv.png';
...
const srv = createServer((req, res) => {
  ...
  // intercepta char-preview.html e usa a versão sem envmap
  if (rel === '/char-preview.html') p = '/tmp/opencode/char-preview-noenv.html';
