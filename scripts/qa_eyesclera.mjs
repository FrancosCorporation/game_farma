// Detecta escleras (branco dos olhos) por COR — capturando os DOIS olhos.
// Uso: node scripts/qa_eyesclera.mjs [modelo=ana] [limiar_min=175] [sat_max=60]
import { chromium } from 'playwright';
import { createServer } from 'node:http';
import { readFileSync, existsSync, statSync } from 'node:fs';
import { join, extname } from 'node:path';
const model = process.argv[2] || 'ana';
const TH = Number(process.argv[3] || 170);
const SATMAX = Number(process.argv[4] || 60);
...
const eyes = await page.evaluate(({ TH, SATMAX }) => {
  ...
  rt.dispose();
  const clusters = [];
  for (let y = Math.floor(H * 0.28); y < H * 0.75; y += 2) {
    for (let x = Math.floor(W * 0.22); x < W * 0.78; x += 2) {
      const i = (y * W + x) * 4;
      const r = buf[i], g = buf[i + 1], b = buf[i + 2];
      const mx = Math.max(r, g, b), mn = Math.min(r, g, b);
      const sat = mx - mn;
      if (r > TH && g > TH - 10 && b > TH - 25 && sat < SATMAX && mx > TH + 10) {
        let found = false;
        for (const c of clusters) {
          if (Math.abs(c.cx - x) < 45 && Math.abs(c.cy - y) < 40) {
