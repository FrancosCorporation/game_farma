// Screenshot com câmera personalizada controlando OrbitControls. char-preview.
import { chromium } from 'playwright';
import { createServer } from 'node:http';
import { readFileSync, existsSync, statSync } from 'node:fs';
import { join, extname } from 'node:path';
const model = process.argv[2] || 'ana';
const out = process.argv[3] || '/tmp/shot_close.png';
const pos = JSON.parse(process.argv[4] || '[0,1.55,0.35]');
const look = JSON.parse(process.argv[5] || '[0,1.55,0]');
const fov = Number(process.argv[6] || 30);
...
await page.evaluate(({ pos, look, fov }) => {
  const camera = window.__camera, controls = window.__controls;
...
console.log('📸', out, 'erros:', errs.length ? errs.join('|') : 'NENHUM');
